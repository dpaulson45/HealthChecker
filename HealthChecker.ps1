...
[Parameter(Mandatory=$true)][object]$AdminDisplayVersion
    ...
    if($AdminDisplayVersion.Major -eq 15 -and $AdminDisplayVersion.Minor -eq 2)
    {
        ...
        if($buildRevision -ge 196.0 -and $buildRevision -lt 221.12) {if($buildRevision -gt 196.0){$exBuildObj.InbetweenCUs = $true} $exBuildObj.CU = [HealthChecker.ExchangeCULevel]::Preview}
